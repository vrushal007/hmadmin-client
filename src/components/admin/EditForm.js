...
function EditForm () {
  ...
  const submitHandler = async e => {
    ...
    if (startDate > dateNow && endDate > startDate) {
      ...
      setIsLoading(false)
      setResponseData(updatedUser.data)
    } else if (startDate < dateNow) {
      setResponseData({ message: 'Cannot book previous dates' })
    } else {
      setResponseData({
        message: 'Start date cannot greater than or equal to end date.'
      })
    }
  ...
  return (
    <div className={classes.form}>
      <form>
        ...
        <input type='date' name='s-date' ref={startDateRef} required />
        <br />
        <label htmlFor='e-date'>End Date</label>
        <input type='date' name='e-date' ref={endDateRef} required />
        <br />
